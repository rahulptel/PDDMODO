...
IloCplex createCplex(IloEnv env) {
	...
	result.setParam(IloCplex::Param::MIP::Limits::Nodes, 1000000000);	//MIP node limit
	result.setParam(IloCplex::Param::MIP::Limits::TreeMemory, 1000000000);	//tree memory limit
	result.setParam(IloCplex::Param::MIP::Tolerances::MIPGap, 0.0);	//relative MIP gap tolerance
	result.setParam(IloCplex::Param::Simplex::Limits::Iterations, 1000000000);	//absolute MIP gap iteration limit
	result.setParam(IloCplex::Param::MIP::Display, 0);	//Display option
	result.setParam(IloCplex::Param::Threads, 1); 		//number of threads
	result.setParam(IloCplex::Param::Advance, 1);
...
void DefiningPoint::ExportNonDominatedPointsToFile(const std::string fileName) {
	...
	outputFile << std::setw(width) << std::setprecision(precision) << std::fixed;
	outputFile << RunTime << " seconds" << std::endl;
